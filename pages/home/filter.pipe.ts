...
export class FilterPipe implements PipeTransform {
  ...
  private filterByOr(item: any, filterGroup: any, searchText: string) {
    let flag = false;
  ...
  private filterByString(item, key, searchText) {
    searchText = searchText.toLowerCase();
    return item[key].toLowerCase().includes(searchText);
  }

  private filterByNumber(item, key, searchText) {
    return item[key] === Number.parseFloat(searchText);
  }

  private filterByBoolean(item, key, searchText) {
